...
public class AggregationBatchService {
    ...
    public void aggregateAfterIngestOrders(Long sid, DateRange input) {
        ...
        Map<LocalDate, List<OrderDelay>> delayOrderMap = query.getDistributeOrder(sid, from, to)
            .stream()
            .collect(Collectors.groupingBy(OrderDelay::getClickDate));

        for (Entry<LocalDate, List<OrderDelay>> entry : delayOrderMap.entrySet()) {
